...
public class SubdivisionAdapter extends RecyclerView.Adapter
{
    ...
    public SubdivisionAdapter(Context context, OrderFragment orderFragment)
    {
        ...
        of             = orderFragment;
        ArrayList<String> element_titles = new ArrayList<>();
        Map<String, ArrayList<CashButtonLayout>> map = new HashMap<>();
        Map<String, Float> costs_map = new HashMap<>();
        items          = new ArrayList<>();
    ...
    public boolean checkIfTotalHasDiscount()
    {
        ...
        if (items.get(0).getDiscount() != 0.0f)
        {
            return true;
        }
        else
...
*/
    ...
    public SubdivisionItem getPerNumberItem()
    {
        ...
        for (int i = 1; i < items.size(); i++)
        {
            if (items.get(i).getMode() == NUMBER_MODE)
            {
                itoReturn = i;
            }
        }
    ...
    {
        if (items.get(items.size() - 1).getMode() == NUMBER_MODE)
        {
            return true;
        }
        else
    ...
    public int addElement(HashMap<CashButtonLayout, Integer> map, Float cost, Object o, int quantity)
    {
        ...
        if (map != null)
        {
            discount = returnDiscountForItem(new ArrayList<>(map.keySet()));
        }
    ...
    public void showFirstPaid()
    {
        if (items.size() > 0)
        {
            if (items.get(0).isPaid() && items.size() > 1)
            {
                showItem(2);
            }
        }
    ...
    public SubdivisionItem addElement(int mode, Float owed_money, Float value, boolean paid, Object map_cbl)
    {
        SubdivisionItem item = new SubdivisionItem();
        item.setMode(mode);
        if (mode == PERCENTAGE_MODE || mode == PARTIAL_MODE)
        {
            item.setPercentage(value);
        }
        else if (mode == NUMBER_MODE)
        {
            item.setNumber_subdivision(value.intValue());
        }
        ...
        if (position < items.size())
        {
            if (position != 0)
            {
                ...
                if ((split_opened_position != -1 && position != 0 || split_opened_position == -1) && !item.isPaid())
                {
                    ...
                    {
                        total_subdivisions_cost -= item.getOwed_money() + returnItemHomage(item);
                    }
                    if (total_subdivisions_cost < 0)
                    {
                        total_subdivisions_cost = 0.0f;
                    }
                    boolean checkAmount = true;
                    for (int x = 0; x < items.size(); x++)
                    {
                        items.get(x).setIsShow(true);
                        if (items.get(x).getMode() == 4)
                        {
                            checkAmount = false;
                        }
                    ...
                    if (checkAmount)
                    {
                        ((PaymentActivity) context).setAllButtonPermission();
                    }
                    ...
                    if (total_bill.getOwed_money() - total_subdivisions_cost + itemsPrice > 0.0f)
                    {
                        if (Float.compare(total_subdivisions_cost, itemsPrice) == 0)
                        {
                            ((PaymentActivity) context).buttonOpacitySetting();
                        }
                        else
                    ...
                    if (items.size() == 1)
                    {
                        of.getOrderListAdapter().setSplit(false);
                    }
