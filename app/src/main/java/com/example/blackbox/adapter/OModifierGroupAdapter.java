...
{
    private static final String TAG = "<OModifierGroupAdapter>";
    ...
    public OModifierGroupAdapter(Context c, DatabaseAdapter dbA, Integer catId, ArrayList<OModifiersGroup> modifiersGroup, RecyclerView parent, Integer groupId)
    {
        ...
        dbA.showDataExec("SELECT * FROM modifiers_group LEFT JOIN modifiers_group_assigned ON modifiers_group.id=modifiers_group_assigned.group_id");

        // TODO : modifiers not working with second query but working with first, why????
        String query = "SELECT DISTINCT modifiers_group.id, modifiers_group.title, modifiers_group.position, modifiers_group.notes " +
                "FROM modifiers_group LEFT JOIN modifiers_group_assigned " +
                "ON modifiers_group.id=modifiers_group_assigned.group_id " +
                "WHERE modifiers_group_assigned.prod_id=" + catId + " AND modifiers_group_assigned.fixed = 0 " +
                "ORDER BY modifiers_group.position";

        modifiers = dbA.fetchOperativeModifiersGroupByQuery(query);
    ...
    {
        final OModifiersGroup groupx      = modifiers.get(position);
        GradientDrawable      border = new GradientDrawable();
        ...
        button.title.setText(groupx.getTitle());
...
//        border.setStroke(3, context.getColor(R.color.gray)); // gray border
        ...
        button.view.setTag(groupx);

        if (groupx.getID() == groupId)
        {
            VibrationClass.vibeOn(context);
            button.view.setActivated(true);
            ModifierFragment.showModifiers(groupx.getID(), catId, groupx.getTitle());
        }
    ...
    public static class OModifiersGroup
    {
        ...
        public boolean getAll_the_group()
        {
            return all_the_group;
        }


        public void setAll_the_group(boolean b)
        {
            all_the_group = b;
        }
