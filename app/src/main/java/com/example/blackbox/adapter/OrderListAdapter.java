...
public class OrderListAdapter extends BaseExpandableListAdapter {
    ...
    public void setBillClose(boolean b){billClose = b;}
    ...
    public float getElementDiscount(int groupPosition) {
        ...
        if(discount==null)discount=0.0f;
        return discount;
    ...
    public int returnQuantity(CashButtonLayout product) {
        ...
        if(homage_list.containsKey(product)) homageQty = homage_list.get(product);
            ...
            if(homage_list.containsKey(product.getOriginalCBL())) homageQty = homage_list.get(product.getOriginalCBL());
        }
        if(discount_list.containsKey(product)) discountQty = discount_list.get(product);
        else if(product.getOriginalCBL()!=null){
            if(discount_list.containsKey(product.getOriginalCBL())) discountQty = discount_list.get(product.getOriginalCBL());
        }
        if(partial_not_yet_saved_quantity.containsKey(product)) pnysq = partial_not_yet_saved_quantity.get(product);
...
*/
    ...
    public void showSplitBill(Map<CashButtonLayout, Integer> split, SubdivisionItem item){
        if(split == null){
            ...
            for (CashButtonLayout prod: groups
                ...
                if(map_backup!=null){
                    if(map_backup.get(p)!=null)map.put(prod, map_backup.get(p));
                    else map.put(prod, prod.getCashList());
                }
                else map.put(prod, prod.getCashList());
    ...
    public ArrayList<CashButtonLayout> showSplitBillToPrintProducts(Map<CashButtonLayout, Integer> split) {
        ...
        for(Map.Entry<CashButtonLayout, Integer>  p : split.entrySet()){
            ...
            else myproducts.remove(p.getKey());
    ...
    public SubdivisionItem getSubdivisionItem(){ return subdivisionItem; }

    public void setSubdivisionItem(SubdivisionItem i){ this.subdivisionItem = i;}
    ...
    public void loadBillSplits(ArrayList<SubdivisionItem> items){
        ...
        else if(!numberSplit) myFragment.setPrice(this.getPartial_cost());
    ...
    public void setDiscountElement(int groupPosition, float amount, boolean reset){
        CashButtonLayout product = groups.get(groupPosition);
        if(reset && amount ==0.0f) product.setNewDiscount(amount);
        else product.setDiscount(amount);
        notifyDataSetChanged();
