...
public class DateUtils
{
    ...
    /**
     * <p>Checks if a calendar date is after now, and it's within a number of minutes from now.</p>
     *
     * @param date the date, not altered, not null
     * @param minutes the number of minutes from now
     * @return true if the calendar input object is withing the next minutes param
     * @throws IllegalArgumentException if the calendar is <code>null</code>
     * */
    public static boolean isWithinMinutesFuture(Date date, int minutes)
    {
        if (date == null)
        {
            throw new IllegalArgumentException("The date must not be null");
        ...
        int diff = ( (cal.get(Calendar.HOUR_OF_DAY) - now.get(Calendar.HOUR_OF_DAY)) * 60 ) + (cal.get(Calendar.MINUTE) - now.get(Calendar.MINUTE));
        return (diff <= minutes);
    }
     ...
     * @throws IllegalArgumentException if the calendar is <code>null</code>
     * */
    public static boolean isWithinMinutesFutureExactly(Date date, int minutes)
    {
        if (date == null)
        {
            throw new IllegalArgumentException("The date must not be null");
        ...
        int diff = ( (cal.get(Calendar.HOUR_OF_DAY) - now.get(Calendar.HOUR_OF_DAY)) * 60 ) + (cal.get(Calendar.MINUTE) - now.get(Calendar.MINUTE));
        return (diff <= (minutes+3) && diff >= (minutes - 3));
    }
